...
class FlightModel extends Model
{
    ...
    function getBookingData($bookingid, $web_partner_id)
    {
        ...
        $builder->select("flight_booking_list.id,flight_booking_list.payment_status,flight_booking_list.coupon_info,flight_booking_list.wl_extra_info,flight_booking_list.is_lcc,flight_booking_list.is_gst_mandatory,flight_booking_list.is_gst_allowed,flight_booking_list.booking_ref_number,flight_booking_list.resultIndex,flight_booking_list.trip_indicator,flight_booking_list.book_request,flight_booking_list.booking_status,flight_booking_list.gst_info,flight_booking_list.tts_search_token,concat('[', group_concat(JSON_OBJECT('id', flight_booking_travelers.id,'title',flight_booking_travelers.title,'first_name',flight_booking_travelers.first_name,'last_name',flight_booking_travelers.last_name,'pax_type',flight_booking_travelers.pax_type,'gendar',flight_booking_travelers.gendar,'date_of_birth',flight_booking_travelers.date_of_birth,'pan_number',flight_booking_travelers.pan_number,'passport_number',flight_booking_travelers.passport_number,'passport_expiry',flight_booking_travelers.passport_expiry,'lead_pax',flight_booking_travelers.lead_pax,'email_id',flight_booking_travelers.email_id,'mobile_number',flight_booking_travelers.mobile_number,'address_1',flight_booking_travelers.address_1,'address_2',flight_booking_travelers.address_2,'city',flight_booking_travelers.city,'country_code',flight_booking_travelers.country_code,'country_name',flight_booking_travelers.country_name,'ff_airline',flight_booking_travelers.ff_airline,'document_number',flight_booking_travelers.document_number,'ff_number',flight_booking_travelers.ff_number,'baggage',flight_booking_travelers.baggage,'passport_issue_date',flight_booking_travelers.passport_issue_date,'meal',flight_booking_travelers.meal,'seat',flight_booking_travelers.seat,'nationality',flight_booking_travelers.nationality) separator ','), ']') as travelersInfo");
    ...
    function getBookingConfirmationData($bookingid, $web_partner_id)
    {
        ...
        if ($query) {
            ...
            $paymentgatewayDetailbuilder->select("web_partner_payment_transaction.payment_status,web_partner_payment_transaction.order_id,web_partner_payment_transaction.amount,web_partner_payment_transaction.payment_response")
                ->join("web_partner_payment_transaction", "web_partner_payment_transaction.booking_ref_no=flight_booking_list.id");
            $paymentgatewayDetailbuilder->where(['flight_booking_list.id' => $bookingid, 'flight_booking_list.web_partner_id' => $web_partner_id, "web_partner_payment_transaction.service" => "flight"]);
