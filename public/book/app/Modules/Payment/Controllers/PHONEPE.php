...
class PHONEPE extends BaseController
{
    ...
    { 
        ...
        $PaymentModel = new PaymentModel();
        $paymentrecord = $PaymentModel->checkpayment_record($this->transaction_table, ['order_id' => $response["transactionId"], 'payment_status' => 'Processing','web_partner_id'=>$this->web_partner_id]);
        ...
        if (!empty($paymentrecord)) {
            ...
            $PaymentModel->updateData($this->transaction_table, array('order_id' => $response["transactionId"]), $updatepaymentdata);
            return array('payment_status' => $payment_status, 'order_id' => $response["transactionId"], 'amount' => $orderAmount);
        } else {
            return array('ErrorCode' => 4001, 'ErrorMessage' => 'Unauthorized request detected', 'order_id' => $response["transactionId"]);
        }
