...
class ICICI extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $gatewayInfo = $PaymentModel->gateway_setting($this->payment_gateway_table, 'ICICI');

        $gatewayInfo = json_decode($gatewayInfo, true);
        $this->MerchantId = $gatewayInfo['merchant_id'];
        $this->WorkingKey = $gatewayInfo['working_key'];
    ...
    public function request($request)
    {
        ...
        $encrypted_data = ICICI::encrypt($merchant_data, $this->WorkingKey);
        $PaymentModel = new PaymentModel();
    ...
    public function response($response)
    {
        ...
        $rcvdString = ICICI::decrypt($encResponse, $this->WorkingKey);
        $order_status = "";
        ...
        if (!empty($paymentrecord)) {
            ...
            return array('payment_status' => $payment_status, 'order_id' => $saveresponse['order_id'], 'amount' => $saveresponse['amount'], 'transaction_id' => $saveresponse['tracking_id'], "tracking_id" => $saveresponse['tracking_id']);
        } else {
    ...
    public function check_status($order_no, $reference_no)
    {
        ...
        for ($i = 0; $i < $dataSize; $i++) {
            ...
            if ($info_value[0] == 'enc_response') {
                $status = ICICI::decrypt(trim($info_value[1]), $this->WorkingKey);
            }
        }
    ...
    function decrypt($encryptedText, $key)
    {
        ...
        $initVector = pack("C*", 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f);
        $encryptedText = ICICI::hextobin($encryptedText);
