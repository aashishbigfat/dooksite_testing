...
class CCAVENUE extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $gatewayInfo = $PaymentModel->gateway_setting($this->payment_gateway_table, 'CCAVENUE');

        $gatewayInfo = json_decode($gatewayInfo, true);
        $this->MerchantId = $gatewayInfo['ccavenue_merchant_id'];
        $this->WorkingKey = $gatewayInfo['ccavenue_working_key'];
        $this->AccessCode = $gatewayInfo['ccavenue_access_code'];
        if (strtolower($gatewayInfo['Mode']) === 'test') {
            $this->PaymentBaseURL = "https://test.ccavenue.com";
            $this->APIStatusURL = "https://apitest.ccavenue.com";
        } else {
            $this->PaymentBaseURL = "https://secure.ccavenue.com/";
            $this->APIStatusURL = "https://api.ccavenue.com";
        }

         $this->MerchantId = "177375";
            $this->WorkingKey = "77BA2B53665AC5B22FCE771055C3CF44";
            $this->AccessCode = "AVJV32KK25BG46VJGB";
            $this->PaymentBaseURL = "https://secure.ccavenue.com/";
            $this->APIStatusURL = "https://api.ccavenue.com";
    }
    ...
    public function request($request)
    {
        ...
        $encrypted_data = CCAVENUE::encrypt($merchant_data, $this->WorkingKey);
        $PaymentModel = new PaymentModel();
        ...
        $data = [
            ...
            'view' => "Payment\Views\ccavenue\index",
        ];
    ...
    public function response($response)
    {
        ...
        $rcvdString = CCAVENUE::decrypt($encResponse, $this->WorkingKey);
        $order_status = "";
        ...
        $status_api_response = CCAVENUE::check_status($saveresponse['order_id'], $saveresponse['tracking_id']);
        ...
        $PaymentModel = new PaymentModel();
        $paymentrecord = $PaymentModel->checkpayment_record($this->transaction_table, ['order_id' => $saveresponse['order_id'], 'payment_status' => 'Processing','web_partner_id'=>$this->web_partner_id]);
        ...
        if (!empty($paymentrecord)) {
            ...
            $PaymentModel->updateData($this->transaction_table, array('order_id' => $saveresponse['order_id']), $updatepaymentdata);
            ...
            return array('ErrorCode' => 4001, 'ErrorMessage' => 'Unauthorized request detected', 'order_id' => $saveresponse['order_id']);
        } 
    }
    ...
    public function check_status($order_no, $reference_no)
    {
        ...
        for ($i = 0; $i < $dataSize; $i++) {
            ...
            if ($info_value[0] == 'enc_response') {
                $status = CCAVENUE::decrypt(trim($info_value[1]), $this->WorkingKey);
            }
        }
    ...
    function decrypt($encryptedText, $key)
    {
        $key = CCAVENUE::hextobin(md5($key));
        $initVector = pack("C*", 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f);
        $encryptedText = CCAVENUE::hextobin($encryptedText);
