...
$(function() {
        ...
        select2search: function(reload = null) {
            ...
            if (reload && reload == true) {
                ...
                $('.select_search').multiselect({
                    ...
                    selectAll: true, // Option to select all items
                    selectAllText: 'Select All', // Text for select all button
                    clearAll: false, // Option to clear all selected items
                    clearAllText: 'Clear All', // Text for clear all button
                    maxHeight: 200, // Maximum height of the dropdown
                    buttonWidth: '100%', // Width of the multi-select button
                    enableFiltering: true, // Enable filtering
                    enableCaseInsensitiveFiltering: true, // Case-insensitive filtering
                    includeSelectAllOption: true, // Include 'Select All' option
                    includeSelectAllIfSelected: true, // Include 'Select All' option only if items are selected
                });
    ...
    $(document).on(
        "keydown",
        "[tts-get-extranet-hotel-name-holiday]",
        function(event) {
            var couterkey = $(this).attr("get-counter-value");
            var hotelCounterValue = $(this).attr("get-hotel-counter-value");
            $(this).autocomplete({
                minLength: 3,
                maxResults: 10,
                open: function() {
                ...
                source: function(request, response) {
                    $.ajax({
                        url: site_url + "holiday/get-hotels",
                        dataType: "json",
                        cache: false,
                        data: {
                            term: request.term,
                ...
                select: function(event, ui) {
                    ...
                    var hotel_id = ui.item.id;
                    var hotel_name = ui.item.hotel_name;
                    var city_id = ui.item.city_id;
                    var address = ui.item.address;
                    var rating = ui.item.hotel_rating;
                    var hotel_image = ui.item.hotel_image;
                    $(
                            "input[name = 'hotel[" +
                            couterkey +
                            "][" +
    ...
    $(document).on(
        "keydown",
        "[tts-get-extranet-activity-name]",
        function(event) {
            var counterKey = $(this).attr("get-counter-value");
            var hotelCounterValue = $(this).attr("get-activity-counter-value");
            ...
            $(this).autocomplete({
                ...
                source: function(request, response) {
                    $.ajax({
                        url: site_url + "holiday/get-acitivity",
                        dataType: "json",
                        cache: false,
                        data: {
                            term: request.term,
                ...
                select: function(event, ui) {
                    ...
                    $('#activity_image_hidden_' + counterKey + '_' + hotelCounterValue).val(ui.item.image);
                    $('#activity_image_preview_' + counterKey + '_' + hotelCounterValue).attr('src', ui.item.image).removeClass('d-none').css('display', 'block');
                    $('.activity_id_' + counterKey + '_' + hotelCounterValue).val(ui.item.id);
                    // 
                    $('#activity_image_uploads_' + counterKey + '_' + hotelCounterValue).css('display', 'none');
                    return false;
                }
            });
        }
    ...
    /*  $(document).on(
         ...
         function (event) {
             ...
             $(this).autocomplete({
                 ...
                 select: function (event, ui) {
                     // Set the selected activity name in the current input
                     $(this).val(ui.item.value);
     
                     // Set the activity image in the corresponding input field
                     $('input[name="activity[' + counterKey + '][' + hotelCounterValue + '][activity_image]"]').val(ui.item.image);
    ...
    $(document).on("submit", "[tts-form='true']", function(e) {
        ...
        $("h6.tts-text-danger", form).html("");
        $("h6.text-danger", form).html("");
        $("span.tts-text-danger", form).html("");
        $("[href]").removeClass("back-praveen-danger");
        $(".boot-multiselect").removeClass("praveen-border");
        var buttontxt;
        ...
        $.ajax({
            ...
            success: function(resp) {
                ...
                if (resp.StatusCode == 1) {
                    let excludedKeys = ['holiday_themes_id.*', 'package_themes_id.*', 'holiday_destination_id.*', 'package_destination_id.*', 'holiday_inclusion_id.*', 'package_inclusion_id.*', 'inclusions.*',
                        'exclusions.*', 'holiday_exclusion_id.*', 'package_exclusion_id.*'
                    ];
                    ...
                    $.each(resp.ErrorMessage, function(key, val) {
                        ...
                        } else if (key == 'package_themes_id.*') {
                            $('[href*="tts-package-themes"]').addClass('back-praveen-danger');

                            $(".tts-package-themes").html('Please select theme').addClass('text-danger');
                        }
                        ...
                        } else if (key == 'package_destination_id.*') {
                            $('[href*="tts-package-destinations"]').addClass('back-praveen-danger');

                            $(".tts-package-destinations").html('Please select destination').addClass('text-danger');
                        }
                        ...
                        } else if (key == 'package_inclusion_id.*') {
                            $('[href*="tts-package-inclusions"]').addClass('back-praveen-danger');

                            $(".tts-package-inclusions").html('Please select inclusion').addClass('text-danger');
                        }
                        ...
                        } else if (key == 'package_exclusion_id.*') {
                            $('[href*="tts-package-exclusion"]').addClass('back-praveen-danger');

                            $(".tts-package-exclusion").html('Please select exclusion').addClass('text-danger');
                        }

                        if (key == 'holiday_description') {
                            $('[href*="tts-package-description"]').addClass('back-praveen-danger');
                        } else if (key == 'package_description') {
                            $('[href*="tts-package-description"]').addClass('back-praveen-danger');
                        }
                        ...
                        if (key == 'visa_detail') {
                            $('[href*="tts-visa-details"]').addClass('back-praveen-danger');
                        }
                        if (key == 'visa_document') {
                            $('[href*="tts-visa-document"]').addClass('back-praveen-danger');
                        }
                        ...
                        if (key.indexOf(".") !== -1) {
                            ...
                            if (!excludedKeys.includes(actualKey)) {
    ...
    $(document).on("click", "[view-data-modal]", function(e) {
        e.preventDefault();
        $("[data-modal-view='view_modal_data']").html('<div class="text-center"><p><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid" width="79" height="79" style="shape-rendering: auto; background: rgb(255, 255, 255);" xmlns:xlink="http://www.w3.org/1999/xlink"><g><circle stroke-width="5" stroke="#000000" fill="none" r="0" cy="50" cx="50"><animate begin="0s" calcMode="spline" keySplines="0 0.2 0.8 1" keyTimes="0;1" values="0;40" dur="1.4705882352941175s" repeatCount="indefinite" attributeName="r"></animate><animate begin="0s" calcMode="spline" keySplines="0.2 0 0.8 1" keyTimes="0;1" values="1;0" dur="1.4705882352941175s" repeatCount="indefinite" attributeName="opacity"></animate></circle><circle stroke-width="5" stroke="#be0027" fill="none" r="0" cy="50" cx="50"><animate begin="-0.7352941176470588s" calcMode="spline" keySplines="0 0.2 0.8 1" keyTimes="0;1" values="0;40" dur="1.4705882352941175s" repeatCount="indefinite" attributeName="r"></animate><animate begin="-0.7352941176470588s" calcMode="spline" keySplines="0.2 0 0.8 1" keyTimes="0;1" values="1;0" dur="1.4705882352941175s" repeatCount="indefinite" attributeName="opacity"></animate></circle><g></g></g></svg></p> <h6> <strong> Please wait a few seconds </strong> </h6></div>');
        ...
        var isAngularImplemented = $(this).attr('data-angular');

        if (view_data_modal == 'true' && modal_id != undefined) {
            ttsopenmodel('view_' + modal_id);
        } else if (view_data_modal == 'true') {
            ...
            success: function(resp) {
                if (resp) {
                    ...
                    if (resp.StatusCode == 0) {
                        ...
                        console.log(isAngularImplemented);

                        if (isAngularImplemented === 'true') {
                            var modalElement = angular.element(document.querySelector('#view_' + modal_id));
                            var modalScope = modalElement.scope();

                            if (modalScope && modalScope.compileModalContent) {
                                modalScope.$applyAsync(function() {
                                    modalScope.setModalData(resp);
                                    modalScope.compileModalContent();
                                });
                            }
                        }
                        ...
                        if (entity_type == "holiday" || entity_type == 'umrah' || (entity_type == 'hajj' && holiday_counter > 0)) {
    ...
    $(document).on("keydown", "[tts-get-airline-multiple]", function(event) {
        ...
        $(this).autocomplete({
            ...
            source: function(request, response) {
                $.ajax({
                    url: site_url + 'flight/get-airline-multiple',
                    dataType: "json",
                    cache: false,
                    data: {
                        term: request.term
...
function tts_searchkey(thisval, formname) {
    ...
    var selectedText = thisval.options[thisval.selectedIndex].innerHTML.trim();
...
function add_more_items(e, id, limit) {
    e.preventDefault();
    if (typeof limit !== 'undefined') {} else {
        limit = 10;
    }
...
function add_more_car_items(e, id, limit) {
    e.preventDefault();
    if (typeof limit !== 'undefined') {} else {
        limit = 10;
    }
...
function remove_more_car_items(thisval, id) {
    var lists = document.querySelectorAll("#" + id + "> .tts-itinerary-row");
    if (lists.length > 1) {
...
function add_more_items_holiday(e, id, limit, site_url) {
    ...
    var variable_html = '' +
        ...
        '' +
        '                                                <button class="badge badge-wt"  href="javascript:void(0);" view-data-modal="true" data-counter="' +
        countval +
        '"   data-controller="holiday" data-href="' + site_url + 'holiday/add-manual-service-template"> Add Manual Services' +
        '                                                </button>' +
        '                                                <button class="badge badge-wt" href="javascript:void(0);" view-data-modal="true" data-counter="' +
        countval +
        '"  data-controller="holiday" data-href="' + site_url + 'holiday/add-auto-service-template"> Select Services' +
        '                                                </button>' +
        '                                                <button class="action badge badge-wt" onclick="remove_more_items_holiday(this,\'package-itinerary-html\')" type="button" ><i class="fa-solid fa-trash"></i></button>' +
        ...
        '                                                    <input class="form-control" type="text" name="itinerary[' + countval + '][itinerary_title]" placeholder="Itinerary Title">' +
...
$(document).on("keypress paste", ".numeric", function(e) {
    if (e.type === "keypress") return (/\d/.test(String.fromCharCode(e.which)));
...
$(document).on("keypress paste", ".decimal", function(e) {
    if (e.type === "keypress") {
        let char = String.fromCharCode(e.which);
        let inputValue = $(this).val();

        return /\d/.test(char) || (char === '.' && !inputValue.includes('.'));
    } else if (e.type === 'paste') {
        let pasteData = e.originalEvent.clipboardData.getData('text');
...
$(document).on("keydown paste", ".no_of_days", function(e) {

    let totalItineraryDays = parseInt($("#total-itinerary-days").val());
    let currentTotalDays = parseInt($("#tts-itinerary-days").val()) || 0;
    ...
    setTimeout(function() {
        ...
        let sumDaysExcludingCurrent = 0;
        $(".no_of_days").each(function() {
            if (this !== e.target) {
                let value = parseInt($(this).val()) || 0;
                sumDaysExcludingCurrent += value;
            }
        });


        let remainingDays = totalItineraryDays - sumDaysExcludingCurrent;
        let currentValue = parseInt($(e.target).val()) || 0;

        if (currentValue > remainingDays) {
            alert('The total number of days cannot exceed ' + totalItineraryDays + '. Remaining days: ' + remainingDays);
            $(e.target).val(remainingDays);
            currentValue = remainingDays;
        }
        let updatedTotalDays = sumDaysExcludingCurrent + currentValue;
        $("#tts-itinerary-days").val(updatedTotalDays);
    }, 0);
});


function add_hajj_itinerary(e, id, limit, site_url) {
    e.preventDefault();

    var totalDaysLimit = parseInt($('#total-itinerary-days').val());
    var totalDaysUsed = parseInt($("#tts-itinerary-days").val()) || 0;


    console.log(totalDaysUsed);
    console.log(totalDaysLimit);

    if (totalDaysUsed >= totalDaysLimit) {
        alert('The total number of itinerary days cannot exceed ' + totalDaysLimit + ' days.');
        return false;
    }
    ...
    var counter = document.getElementById('tts-hajj-counter').value;
    var countval = Number(counter) + 1;
    document.getElementById('tts-hajj-counter').value = countval;

    var variable_html = `
        <div class="tts-itinerary-row">
            <input type="hidden" name="itinerary[${countval}][itinerary_no]" value="${countval}">
            <div class="accordion" id="tts-package-accordion">
                <div class="accordion-item">
                    <h2 class="accordion-header" id="headingDay${countval}">
                        <button class="accordion-button text-bold" type="button" data-bs-toggle="collapse"
                        data-bs-target="#collapseDay${countval}" aria-expanded="true" aria-controls="collapseDay${countval}">
                        Itinerary ${countval}
                        </button>
                    </h2>
                    <div id="collapseDay${countval}" class="accordion-collapse collapse show" aria-labelledby="headingDay${countval}"
                        data-bs-parent="#tts-package-accordion">
                        <div class="accordion-body">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="row">
                                        <div class="col-md-12 text-end mb-2">
                                            <button class="badge badge-secondary" view-data-modal="false" data-counter="${countval}"
                                            data-controller="hajj" itinerary-service="flight"
                                            data-href="${site_url}hajj/add-itinerary-flight-template">
                                                <i class="fa-solid fa-add"></i> Flight
                                            </button>
                                            <button class="badge badge-secondary" view-data-modal="false"
                                                data-controller="hajj" data-counter="${countval}" itinerary-service="hotel"
                                                data-href="${site_url}hajj/add-itinerary-hotel-template">
                                                <i class="fa-solid fa-add"></i> Hotel
                                            </button>
                                            <button class="badge badge-secondary" view-data-modal="false"
                                                data-controller="hajj" data-counter="${countval}" itinerary-service="transfer"
                                                data-href="${site_url}hajj/add-itinerary-transfer-template">
                                                <i class="fa-solid fa-add"></i> Transfer
                                            </button>
                                            <button class="badge badge-secondary" view-data-modal="false"
                                                data-controller="hajj" data-counter="${countval}" itinerary-service="activity"
                                                data-href="${site_url}hajj/add-itinerary-activity-template">
                                                <i class="fa-solid fa-add"></i> Activity
                                            </button>
                                            <button class="action badge badge-dgr"
                                                onclick="remove_hajj_itinerary(this,'hajj-itinerary-html')" type="button">
                                    ...
                                    <div class="row">
                                        <div class="col-md-3">
                                            <div class="form-group form-mb-20 no-zindex">
                                                <label>City *</label>
                                                <select class="form-control" name="itinerary[${countval}][city]" tts-select2-ajax="true"
                                                    tts-method-nam="hajj/get-cities" placeholder="Itinerary City">
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group form-mb-20">
                                                ...
                                                <input class="form-control numeric no_of_days" type="text"
                                                    name="itinerary[${countval}][no_of_days]" placeholder="No Of Days" value="1">
                                            </div>
                                        </div>
                                        <div class="col-md-3">
                                            <div class="form-group form-mb-20">
                                                <label>Itinerary Title *</label>
                                                <input class="form-control" type="text" name="itinerary[${countval}][itinerary_title]"
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group form-mb-20">
                                                <label>Meal *</label>
                                                <select class="form-control select_search no-zindex"
                                                    name="itinerary[${countval}][meal_type]" placeholder="Meal">
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group form-mb-20">
                                                <label>Itinerary Image</label>
                                                <input class="form-control" type="file" name="itinerary[${countval}][image]"
                                                    placeholder="Itinerary Image">
                                            </div>
                                        </div>
                                        <div class="col-md-12">
                                            <div class="form-group form-mb-20">
                                                <label>Description</label>
                                                <textarea class="form-control" name="itinerary[${countval}][description]" rows="3"
                                                    placeholder="Description"></textarea>
                                            </div>
                                        </div>
                                    </div>
                                    <div tts-append-html="true" class="tts-append-html-counter${countval}"></div>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    document.getElementById(id).insertAdjacentHTML('beforeend', variable_html);
    totalDaysUsed += 1;
    $("#tts-itinerary-days").val(totalDaysUsed);
...
function remove_more_items_hajj(thisval, id) {
    var lists = document.querySelectorAll("#" + id + "> .tts-itinerary-row");
    if (lists.length > 1) {
        if (confirm('Do you want to delete item ?')) {
            $(thisval).parent().parent().parent().parent().parent().remove();
...
function remove_more_items_hajjs(thisval, id, itinerary_id) {
    var lists = document.querySelectorAll("#" + id + " > .tts-itinerary-row");
    ...
    if (lists.length > 1) {
        if (confirm("Do you want to delete this item?")) {
            ...
            $.ajax({
                type: 'POST',
                url: site_url + "hajj/remove-hajj-itinerary",
                data: { itinerary_id: itinerary_id },
                success: function(response) {
                    if (response.StatusCode == 0) {
                        $(thisval).closest('.tts-itinerary-row').remove();

                        lists = document.querySelectorAll("#" + id + " > .tts-itinerary-row");
                        lists.forEach(function(item, index) {
                            var countval = index + 1;

                            var header = item.querySelector('.accordion-header button');
                            if (header) {
                                header.innerHTML = "Itinerary " + countval;
                                header.setAttribute("data-bs-target", "#collapseDay" + countval);
                                header.setAttribute("aria-controls", "collapseDay" + countval);
                            }

                            var collapse = item.querySelector('.accordion-collapse');
                            if (collapse) {
                                collapse.id = "collapseDay" + countval;
                                collapse.setAttribute("aria-labelledby", "headingDay" + countval);
                            }

                            var heading = item.querySelector('.accordion-header');
                            if (heading) {
                                heading.id = "headingDay" + countval;
                            }

                            var textarea = item.querySelector('textarea');
                            if (textarea) {
                                textarea.name = "itinerary[" + countval + "][description]";
                            }

                            var appendHtml = item.querySelector('[tts-append-html="true"]');
                            if (appendHtml) {
                                appendHtml.className = "tts-append-html-counter" + countval;
                            }
                        });

                        var counter = parseInt(document.getElementById('tts-hajj-counter').value) || 0;
                        counter = counter > 0 ? counter - 1 : 0;
                        document.getElementById('tts-hajj-counter').value = counter;

                        var totalDaysUsed = parseInt($("#tts-itinerary-days").val()) || 0;
                        totalDaysUsed = totalDaysUsed > 0 ? totalDaysUsed - 1 : 0;
                        $("#tts-itinerary-days").val(totalDaysUsed);
        ...
        alert("You can't delete the last remaining item.");
    }
}


function remove_more_items_services_hajj(thisval, id) {
    if (confirm('Do you want to delete item ?')) {
        $(thisval).parent().parent().parent().parent().parent().remove();
    }
}
...
$(document).on("focus", "[package-from-date]", function() {
    $(this).datepicker({
        dateFormat: 'dd M yy',
        changeMonth: true,
        changeYear: true,
...
function updateReturnDate() {
    const departureDateInput = $("[package-from-date]");
    const returnDateInput = $("[package-return-date]");
    const noOfNightsInput = $("input[name='no_of_nights']");

    const departureDate = departureDateInput.val();
    const noOfNights = parseInt(noOfNightsInput.val(), 10);

    if (departureDate && noOfNights && noOfNights > 0) {
        const fromDate = $.datepicker.parseDate('dd M yy', departureDate);
        const minReturnDate = new Date(fromDate);
        minReturnDate.setDate(fromDate.getDate() + noOfNights);

        returnDateInput.datepicker("option", "minDate", minReturnDate);

        const currentReturnDate = returnDateInput.val();
        if (currentReturnDate) {
            const parsedReturnDate = $.datepicker.parseDate('dd M yy', currentReturnDate);
            if (parsedReturnDate < minReturnDate) {
                returnDateInput.val('');
            }
        }
    } else {
        returnDateInput.val('').datepicker("option", "minDate", null);
    }
}


$(document).on("focus", "[package-return-date]", function() {
    const departureDateInput = $("[package-from-date]");
    const noOfNightsInput = $("input[name='no_of_nights']");
    const departureDate = departureDateInput.val();
    const noOfNights = parseInt(noOfNightsInput.val(), 10);
...
$(document).on("change", "[package-from-date], input[name='no_of_nights']", function() {
    updateReturnDate();
});
...
$(document).on("focus", "[price-calendar-from]", function() {
    let minStartDate = $(this).attr("min-value");
    ...
    $(this).datepicker({
        ...
        onClose: function(selectedDate) {
            if (selectedDate) {
                let newdate = new Date(selectedDate);
                newdate.setDate(newdate.getDate() + 1);
                $("[price-calendar-to]").prop("disabled", false);
                $("[price-calendar-to]").datepicker("option", "minDate", newdate);
                $("[price-calendar-to]").focus();
            }
        }
    });
});
...
$(document).on("focus", "[price-calendar-to]", function() {
    if (!$(this).prop("disabled")) {
        $(this).datepicker({
            ...
            numberOfMonths: 2,
            beforeShow: function() {
                let seldate = $("[price-calendar-from]").val();
                let newdate = new Date(seldate);
                newdate.setDate(newdate.getDate() + 1);
                $(this).datepicker("option", "minDate", newdate);
            },
        });
...
$(document).on("focus", "[start-date-coupon]", function() {
    $("[start-date-coupon]").datepicker({
        dateFormat: 'dd M yy',
        changeMonth: true,
        changeYear: true,
        minDate: "0",
        ...
        onClose: function(selectedDate) {
            if (selectedDate) {
                let newdate = new Date(selectedDate);
                newdate.setDate(newdate.getDate() + 1);
                $("[end-date-coupon]").prop("disabled", false);
                $("[end-date-coupon]").datepicker("option", "minDate", selectedDate).focus().select();
            }
        }
    });
});

$(document).on("focus", "[end-date-coupon]", function() {
    $("[end-date-coupon]").datepicker({
        dateFormat: 'dd M yy',
        changeMonth: true,
        changeYear: true,
        minDate: "0",
        yearRange: "-0:+2",
        beforeShow: function() {
            var selectedDate = $('[start-date-coupon]').val();
            var newdate = selectedDate.split("-").reverse().join("-");
            var newdate = new Date(newdate);
            newdate.setDate(newdate.getDate() + 1);
            $(this).datepicker("option", "minDate", newdate);
        ...
        onClose: function(selectedDate) {
            $("[start-date-coupon]").datepicker("option", selectedDate);
        }
    });
});
/** JS For Hajj Ends */
...
$(document).on("keydown", "[holiday-airport-city]", function(event) {
    ...
    $(this).autocomplete({
        ...
        source: function(request, response) {
            $.ajax({
                url: site_url + 'holiday/get-airport-city',
                dataType: "json",
                cache: false,
                data: {
                    term: request.term
        ...
        select: function(event, ui) {
            $(this).val(ui.item.city);
            $(this).closest('.form-control').siblings('[holiday-flight-detail="true"]').val(JSON.stringify(ui.item));
            $(this).closest('.form-control').siblings('[holiday-flight-detail-auto="true"]').val(ui.item.city_code);

            $('[name="city_code"]').val(ui.item.city_code);
            $('[name="country_name"]').val(ui.item.country_name);
            $('[name="country_code"]').val(ui.item.country_code);
            $('[name="airport_name"]').val(ui.item.airport_name);
            $('[name="airport_code"]').val(ui.item.airport_code);
            return false;
        },
        change: function(event, ui) {
            $(this).val((ui.item ? ui.item.city : ""));
...
function add_umrah_itinerary(e, id, limit, site_url) {
    e.preventDefault();
    ...
    var counter = document.getElementById('tts-umrah-counter').value;
    var countval = Number(counter) + 1;
    document.getElementById('tts-umrah-counter').value = countval;
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="row">
                                        <div class="col-md-12 text-end mb-2">
                                            <button class="badge badge-secondary" view-data-modal="false" data-counter="${countval}"
                                            data-controller="umrah" itinerary-service="flight"
                                            data-href="${site_url}umrah/add-itinerary-flight-template">
                                                <i class="fa-solid fa-add"></i> Flight
                                            </button>
                                            <button class="badge badge-secondary" view-data-modal="false"
                                                data-controller="umrah" data-counter="${countval}" itinerary-service="hotel"
                                                data-href="${site_url}umrah/add-itinerary-hotel-template">
                                                <i class="fa-solid fa-add"></i> Hotel
                                            </button>
                                            <button class="badge badge-secondary" view-data-modal="false"
                                                data-controller="umrah" data-counter="${countval}" itinerary-service="transfer"
                                                data-href="${site_url}umrah/add-itinerary-transfer-template">
                                                <i class="fa-solid fa-add"></i> Transfer
                                            </button>
                                            <button class="badge badge-secondary" view-data-modal="false"
                                                data-controller="umrah" data-counter="${countval}" itinerary-service="activity"
                                                data-href="${site_url}umrah/add-itinerary-activity-template">
                                                <i class="fa-solid fa-add"></i> Activity
                                            </button>
                                            <button class="action badge badge-dgr"
                                                onclick="remove_umrah_itinerary(this,'umrah-itinerary-html')" type="button">
                                                <i class="fa-solid fa-trash"></i>
                                            </button>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-3">
                                            <div class="form-group form-mb-20 no-zindex">
                                                <label>City *</label>
                                                <select class="form-control" name="itinerary[${countval}][city]" tts-select2-ajax="true"
                                                    tts-method-nam="umrah/get-cities" placeholder="Itinerary City">
                                                    <option value="">Select City</option>
                                                </select>
                                            </div>
                                        </div>
...
function remove_umrah_itinerary(thisval, id) {
    var lists = document.querySelectorAll("#" + id + " > .tts-itinerary-row");
    ...
    if (lists.length > 1) {
        if (confirm("Do you want to delete this item?")) {
            ...
            var counter = parseInt(document.getElementById('tts-umrah-counter').value) || 0;
            counter = counter > 0 ? counter - 1 : 0;
            document.getElementById('tts-umrah-counter').value = counter;
...
function remove_umrah_itinerary_exist(thisval, id, itinerary_id) {
    var lists = document.querySelectorAll("#" + id + " > .tts-itinerary-row");
    ...
    if (lists.length > 1) {
        if (confirm("Do you want to delete this item?")) {
            ...
            $.ajax({
                type: 'POST',
                url: site_url + "umrah/remove-umrah-itinerary",
                data: { itinerary_id: itinerary_id },
                success: function(response) {
                    if (response.StatusCode == 0) {
                        $(thisval).closest('.tts-itinerary-row').remove();
