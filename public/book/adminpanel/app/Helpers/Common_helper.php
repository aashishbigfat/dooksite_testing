...
if (!function_exists('active_list_mod')) {
    function active_list_mod($controller_name, $function_name)
    {
        ...
        $uri = $request->getUri(); // Get the URI object
        if ($controller_name == $getcontrollername) {
            if ($uri->getTotalSegments() >= 3 && $uri->getSegment(3)) {
                if ($uri->getSegment(3) == $function_name) {
                    echo "active";
                }
            } else {
...
if (!function_exists('send_email')) {
    function send_email($to, $subject, $message, $email_type = null, $attachment = null, $extraprameter = array(), $param1 = null)
    {
        ...
        if (!empty($extraprameter)) {
            ...
            $booking_id = $extraprameter['booking_id'];
            $sending_type = $extraprameter['sending_type'];
            $ccemail = isset($extraprameter['ccemail']) ? $extraprameter['ccemail'] : "";
        }

        $from_email = $email_settings['from_email'];
        $ccss = isset(super_admin_website_setting['cc_email']) ? super_admin_website_setting['cc_email'] : '';
        $cc = (!empty($ccemail)) ? $ccemail : $ccss;
        $bcc = isset(super_admin_website_setting['bcc_email']) ? super_admin_website_setting['bcc_email'] : '';
...
/**
 * -------------------------------------
 * Encode Function modify By Abhay
 * -------------------------------------
 */

/* function derive_key(string $key, string $salt = "Abhay@!@#353$#%#", int $iterations = 100, int $key_length = 50): string
{
    return hash_pbkdf2("sha1", $key, $salt, $iterations, $key_length, true);
} */
...
/* if (!function_exists('dev_encode')) {
    function dev_encode($string, $key = "", $url_safe = true)
    {
        ...
        $encrypted = openssl_encrypt($string, "AES-256-CBC", $hashed_key, OPENSSL_RAW_DATA, $iv);
        if ($encrypted === false) {
            return null;
        }
...
/**
 * -------------------------------------
 * Decode  Function modify By Abhay
 * -------------------------------------
 */
...
/* if (!function_exists('dev_decode')) {
    function dev_decode($string, $key = "")
    {
        ...
        $decrypted = openssl_decrypt($encrypted, "AES-256-CBC", $hashed_key, OPENSSL_RAW_DATA, $iv);
        if ($decrypted === false) {
            return null;
        }
...
/**
 * -------------------------------------
 * Create Date Format  Function Modify my Abhay
 * -------------------------------------
 */
...
if (!function_exists('timestamp_to_date')) {
    ...
    {
        if (!is_numeric($created_date) || $created_date <= 0) {
            return " ";
        }
        $admin_data = admin_cookie_data();
        $timezone = 'UTC'; // Default timezone
...
if (!function_exists('admin_cookie_data')) {
    function admin_cookie_data()
    {
        ...
        if (!empty($crm_user)) {
            $crm_comapny_detail = '';
            if (dev_decode(session()->get('admin_comapny_detail'))) {
                $crm_comapny_detail = json_decode(dev_decode(session()->get('admin_comapny_detail')), true);
            }
            $crm_user_details_data = '';
            if (dev_decode(session()->get('admin_user_details'))) {
...
if (!function_exists('image_upload')) {
    function image_upload($file, $field_name, $upload_folder, $resizeDim, $aspect_ratio = true, $reference = 'height', $size = 1024): array
    {
        ...
        $allowedMimeTypes = ['image/jpg', 'image/jpeg', 'image/png', 'image/ico', 'image/icon', 'image/x-icon', 'text/ico', 'application/ico', 'image/svg', 'image/svg+xml', 'image/webp', 'image/avif', 'application/pdf'];
        if (is_array($file)) {
            // Code for multiple file uploads
            $validation->setRules([
                $field_name => [
                    "uploaded[$field_name].0",
                    "mime_in[$field_name," . implode(',', $allowedMimeTypes) . "]",
                    "max_size[$field_name,$size]",
                ]
            ]);
            ...
            if ($validation->withRequest($request_data)->run()) {
                foreach ($file as $key => $img) {
                    if ($img->isValid() && !$img->hasMoved()) {
                        ...
                        $mineTypeee = $img->getMimeType();
                        $newNameRandom = create_date() . '_' . str_replace(['(', ')'], '', strtolower($ImageName));

                        if ($img->move(FCPATH . "../uploads/$upload_folder/", $newNameRandom)) {
                            // Skip resizing for SVG and PDF files
                            if (in_array($mineTypeee, ['image/svg', 'image/svg+xml', 'application/pdf', 'image/avif', 'image/webp'])) {
                            ...
                            if (!in_array($mineTypeee, ['image/svg', 'image/svg+xml', 'application/pdf', 'image/avif', 'image/webp'])) {
                                ...
                                $image = service('image');
                                $image->withFile($path)
                                    ->resize($resizeDim['width'], $resizeDim['height'], $aspect_ratio, $reference)
                                    ->save($thumbpath);
                            }
...
if (!function_exists('User_Query_date')) {
    function User_Query_date(string $date_string): int
    {
        $timestamp = strtotime($date_string);
        if ($timestamp == false) {
            return 0;
        }
        $formatted_date = date('Y-m-d G:i', $timestamp);
...
if (!function_exists('show_booking_currency_amounts')) {
    function show_booking_currency_amounts($Amount, $BookingCurrency = "INR", $CurrencyRate = 1)
    {
        $session = session();
        $currencyData = $session->get('currencyinfo');

        if (isset($currencyData[$BookingCurrency])) {
            $DecimalPoints = $currencyData[$BookingCurrency]['decimal_point'];
            $CurrencySymbol = $currencyData[$BookingCurrency]['currency_symbol'];
        } else {
            $DecimalPoints = 2;
            $CurrencySymbol = '₹';
        }
        ...
        $TotalAmount = ($Amount * $CurrencyRate);
        $finalAmounts = $CurrencySymbol . ' ' . number_format($TotalAmount, $DecimalPoints);
        return $finalAmounts;
    }
}
...
if (!function_exists('booking_currency_refunds')) {
    function booking_currency_refunds($Amount, $BookingCurrency = "INR", $CurrencyRate = 1, $extraprameter = array())
    {
        $session = session();
        $currencyData = $session->get('currencyinfo');
        if (isset($currencyData[$BookingCurrency])) {
        ...
        if ($BookingCurrency == NULL || $BookingCurrency == "INR") {
            $InrAmounts = ($Amount * $CurrencyRate);
            $TotalAmount = round_value($InrAmounts, $DecimalPoints);
        } else {
            $TotalAmount = ($Amount * $CurrencyRate);
        }
        return $TotalAmount;
...
if (!function_exists('initialize_error_handler')) {
...
if (!function_exists('generateConfirmationUniqueCode')) {
    function generateConfirmationUniqueCode($length = 8)
    {
        $nanoTime = sprintf("%015d", hrtime(true)); 
        $microTime = sprintf("%06d", (int)(microtime(true) * 1000000) % 1000000);
        $characters = time() . 'ABCDEFGHIJKLM0908171262383404595NOPQRSTUVWXYZ0123456789' . date('dmy');
        $randomCode = '';
        for ($i = 0; $i < $length; $i++) {
            $randomCode .= $characters[random_int(0, strlen($characters) - 1)];
        }

        /* $code = 'TTS-' . date('dmy') . '-' . $microTime  . substr($nanoTime, -6) . $randomCode;  */
        
        $code = 'TTS' . date('dmy') . '-' . substr($nanoTime, -6) . $randomCode; 
        return $code;
...
if (!function_exists('setImageValidation')) {
    function setImageValidation($validate, $validationType, $field, $size, $label, $permitEmpty = false)
    {
        $rules = $permitEmpty ? "permit_empty|uploaded[$field]" : "uploaded[$field]";
        $rules .= "|max_size[$field,$size]|mime_in[$field,image/jpg,image/jpeg,image/png,image/webp,image/gif,image/svg]";

        $validate->$validationType[$field] = [
            'label' => $label,
            'rules' => $rules,
            'errors' => [
                'max_size' => "$label size should not be more than $size Kb.",
                'mime_in' => "Please upload a valid $label. Allowed types are jpg, jpeg, png, webp, gif, svg."
            ],
        ];
    }
}
...
if (!function_exists('handleImageUpload')) {
    function handleImageUpload($fieldName, $imgInfo, $maxSize, $folderName)
    {
        $request = service('request');
        $file = $request->getFile($fieldName);
        if (!$file->isValid()) {
            return ['status_code' => 1, 'message' => 'Invalid file upload.'];
        }

        $resizeDim = ['width' => 350, 'height' => 250];
        $aspectRatio = false;
        $referenceTo = 'height';

        if (!empty($imgInfo['thumbnail'])) {
            $thumbnail = $imgInfo['thumbnail'];
            $resizeDim = [
                'width' => $thumbnail['width'] ?? $resizeDim['width'],
                'height' => $thumbnail['height'] ?? $resizeDim['height']
            ];
            $aspectRatio = $thumbnail['aspect_ratio'] ?? $aspectRatio;
            $referenceTo = $thumbnail['reference_to'] ?? $referenceTo;
        }

        return image_upload($file, $fieldName, $folderName, $resizeDim, $aspectRatio, $referenceTo, $maxSize);
    }
}



if (!function_exists('image_size_label')) {
    function image_size_label($image_info)
    {
        if (!empty($image_info['main'])) {
            if (!empty($image_info['main']['label'])) {
                return $image_info['main']['label'];
            }

            $height = $image_info['main']['height'] ?? '';
            $width = $image_info['main']['width'] ?? '';

            if (!empty($width) && !empty($height)) {
                return "(Image Resolution Size $width * $height)";
            }
        }
        return "";
    }
...
if(!function_exists('car_booking_date_time')) {
    function car_booking_date_time($date, $time) {
        return date("d M Y", strtotime($date)) ." / ". date("g:ia", strtotime($time));
    }
}
