...
class FlightExtranet extends BaseController
{
    ...
    public function initController(\CodeIgniter\HTTP\RequestInterface $request, \CodeIgniter\HTTP\ResponseInterface $response, \Psr\Log\LoggerInterface $logger)
    {
        ...
        $this->admin_name = $this->web_partner_details['first_name'] . " " . $this->web_partner_details['last_name'];
    ...
    public function private_fare_list(): string
    {
        try {
            if (permission_access_error("FlightExtranet", "private_fare_list")) {
                ...
                $data = array("StatusCode" => 2, "Message" => "Permission Denied", "Class" => "error_popup");
            }
        } catch (Throwable $e) {
            log_error_details($e);
            $data = [
                'view' => "FlightExtranet\Views/exception_view",
        ...
        try {
            if (permission_access("FlightExtranet", "add_private_fare")) {
                ...
                if (!$rules) {
                    ...
                    $flightcrslist = $FlightCrsFareListModel->flight_crs_list($this->web_partner_id);
                    ...
                    $data['update_user'] = $this->admin_name;
                    $checkInventoryExist = $this->check_duplicate_flight($flightcrslist, $data);
                    if ($checkInventoryExist) {
                        $message = array("StatusCode" => 2, "Message" => "Inventory already exists.", "Class" => "error_popup", "Reload" => "true");
    ...
    public function check_duplicate_flight($flightcrslist, $request_data)
    {
        if (!empty($flightcrslist)) {
            foreach ($flightcrslist as $flight) {
                if (
                    ($flight['origin'] == $request_data['origin']) &&
                    ($flight['destination'] == $request_data['destination']) &&
                    ($flight['trip_type'] == $request_data['trip_type']) &&
                    ($flight['journey_type'] == $request_data['journey_type'])
                ) {
                    $data_base_segment = json_decode($flight['onward_segment_detail'], true);
                    $request_segment = json_decode($request_data['onward_segment_detail'], true);
                    foreach ($data_base_segment as $db_segments) {
                        foreach ($db_segments as $db_flight) {
                            foreach ($request_segment as $req_segments) {
                                foreach ($req_segments as $req_flight) {
                                    if (
                                        $db_flight['airline_code'] == $req_flight['airline_code'] &&
                                        $db_flight['flight_number'] == $req_flight['flight_number']
                                    ) {
                                        return true;
                                    }
                                }
        ...
        try {
            if (permission_access("FlightExtranet", "edit_private_fare")) {
                ...
                if (!$rules) {
                    ...
                    $table_name = 'flight_crs_fare_list';
                    $condn = ['web_partner_id' => $this->web_partner_id, 'id!=' => $id];
                    $flightcrslist = $FlightCrsFareListModel->getRowData($table_name, $condn);
                    $checkInventoryExist = $this->check_duplicate_flight($flightcrslist, $data);
                    if ($checkInventoryExist) {
                        $message = array("StatusCode" => 2, "Message" => "Inventory already exists.", "Class" => "error_popup", "Reload" => "true");
                        $this->session->setFlashdata('Message', $message);
    ...
    public function add_seat_allocation()
    {
        if (permission_access("FlightExtranet", "add_seat_allocation")) {
            ...
            if ($data['flight_type'] == 'group') {
                $validate->seat_allocation['closing_date'] = [
                    'label' => 'Closing Date',
                    'rules' => 'required'
                ];
                $validate->seat_allocation['advance_amount'] = [
                    'label' => 'Advance Amount',
                    'rules' => 'required|numeric|greater_than_equal_to[1]'
                ];
                $validate->seat_allocation['minm_seats'] = [
                    'label' => 'Minimum Required Seats',
                    'rules' => 'required|numeric|greater_than_equal_to[1]'
                ];
                $validate->seat_allocation['moq'] = [
                    'label' => 'MOQ',
                    'rules' => 'required|numeric|greater_than_equal_to[1]'
                ];
                unset($validate->seat_allocation['end_date']);
            }
            ...
            if (!$rules) {
                ...
                $data['update_user'] = $this->admin_name;
                $data['is_deal'] = isset($data['is_deal']) ? 1 :0;
        ...
        if (permission_access("FlightExtranet", "edit_seat_allocation")) {
            ...
            if (!$rules) {
                ...
                if ($added_data) {
                    ...
                    $message = array("StatusCode" => 2, "Message" => "seat allocation not updated. Date can't be changed", "Class" => "error_popup", "Reload" => "true");
                }
                $this->session->setFlashdata('Message', $message);
                return $this->response->setJSON($message);
            }
            ...
            if (!$rules) {
                ...
                if (!empty($date_period)) {
                    foreach ($date_period as $key => $period) {
                        ...
                        if ($details) {
                            $added_data = $FlightCrsSeatAllocationModel->where(["date" => $period, "web_partner_id" => $this->web_partner_id, "id" => $id,])->set($data)->update();
                        }
                    }
                }
    ...
    public function seat_allocation_status_change()
    {
        //if (permission_access("FlightExtranet", "fare_rule_status")) {
        $validate = new Validation();
        $rules = $this->validate($validate->status);
        if (!$rules) {
            ...
            $ids = $this->request->getPost('checkedvalue');
            $data['status'] = $this->request->getPost('status');
            $update = $FlightCrsSeatAllocationModel->seat_allocation_status_change($ids, $data, $this->web_partner_id);
