...
class FlightAirportModel extends Model
{
    ...
    public function get_airport_autosuggestion($data)
    {
        ...
        if (!empty($data) && $data != null) {
            return $this->select('*')
                ->like("code", $data)
                ->orLike('city_name', $data)
                ->orLike('country_name', $data)
                ->orLike('country_code', $data)
                ->orderBy("
                        CASE 
                            WHEN code LIKE '%" . $data . "%' THEN 1
                            WHEN city_name LIKE '%" . $data . "%' THEN 2
                            WHEN country_name LIKE '%" . $data . "%' THEN 3
                            WHEN country_code LIKE '%" . $data . "%' THEN 4
                            ELSE 5
                        END")
                ->limit(20)
                ->get()
            ...
            return  $this->select('*')->where("airportorder!=", null)->orderBy('airportorder', "asc")->limit(20)->get()->getResultArray();
        }
    }
//     public function get_airport_autosuggestion($data)
//     {
