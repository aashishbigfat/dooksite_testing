...
namespace Modules\GroupFlight\Controllers;

use App\Modules\GroupFlight\Models\GroupFlightModel;
use App\Modules\GroupFlight\Models\GroupFlightBookingModel;
use App\Modules\GroupFlight\Models\GroupFlightAmendmentModel;
use App\Modules\GroupFlight\Models\FlightAirportModel;
use App\Modules\GroupFlight\Models\FlightAirlineModel;
use App\Modules\GroupFlight\Models\GroupFlightMarkupModel;
use App\Modules\GroupFlight\Models\AgentClassModel;
use App\Modules\GroupFlight\Models\FlightDiscountModel;
use App\Modules\GroupFlight\Models\DistributorModel;
use App\Modules\GroupFlight\Models\SupplierModel;
use App\Models\CommonModel;
use App\Controllers\BaseController;
use Modules\GroupFlight\Config\Validation;

class GroupFlight extends BaseController
{
	...
	public function initController(\CodeIgniter\HTTP\RequestInterface $request, \CodeIgniter\HTTP\ResponseInterface $response, \Psr\Log\LoggerInterface $logger)
	{
		...
		helper('Modules\GroupFlight\Helpers\flight');

		$this->Services = API_REQUEST_URL . '/groupflight/rest/';
	}
	...
	function groupflight_booking_calender()
	{
		$getData = $this->request->getGet();
		$GroupFlightBookingModel = new GroupFlightBookingModel();
		$booking_list = $GroupFlightBookingModel->getCalenderList($this->web_partner_id, $getData);
		$booking_result = [];
		if ($booking_list) {
			foreach ($booking_list as $key => $booking) {
				...
				if ($booking['journey_type'] == "RoundTrip" && $booking['is_domestic'] == 1) {
					$return_data = $GroupFlightBookingModel->getReturnBookingDetail($this->web_partner_id, $booking['tts_search_token'], $getData);
		...
		$data = [
			'title' => $this->title,
			'view' => "GroupFlight\Views\listing/group-flight-booking-calender",
			"list" => $booking_result,
			"search_bar_data" => $getData,
			'pager' => $GroupFlightBookingModel->pager,
		];
	...
	function confirmation()
	{
		...
		$GroupFlightModel = new GroupFlightModel();
		...
		if ($bookingIds) {
			...
			foreach ($bookingIds as $bookingId) {
				...
				$FareBreakUp = array();
				$couponAmount = 0;
				$bookingInfo = $GroupFlightModel->getBookingConfirmationData($bookingId, $this->web_partner_id);
		...
		$data = [
			...
			'view' => "GroupFlight\Views\listing\booking-confirmation-page",
		];
		return view('template/sidebar-layout', $data);
	}
		...
		if (!$this->request->isAJAX()) {
			...
			if ($bookingRefNumbers) {
				foreach ($bookingRefNumbers as $BookingRefNumber) {
					$bookingInfoData = $GroupFlightModel->getBookingWithBookingRefNumberWithVariableFieldNameData($BookingRefNumber, $this->web_partner_id, "id,booking_source,tts_search_token");
					if ($bookingInfoData) {
				...
				if ($bookingRefNumbers && $bookingInfo) {
					...
					$data = [
						'title' => $this->title,
						'view' => "GroupFlight\Views\listing\print_ticket",
						'data' => $response['Result']['Html'],
					];
					return view('Modules\GroupFlight\Views\listing\print_ticket', $data);
				} else {
					return $this->response->redirect(site_url('groupflight/error?errormessage=Request Not Allowed'));
				}
			} else {
				return $this->response->redirect(site_url('groupflight/error?errormessage=Request Not Allowed'));
			}
	...
	function bookingLists()
	{
		...
		if (isset($getData['key'])) {
			$list = $GroupFlightBookingModel->search_bookings($getData, $this->web_partner_id, $this->web_partner_details['id'], $this->web_partner_details['primary_user'], $bookingType, $source);
		} else {
			...
			$list = $GroupFlightBookingModel->flight_booking_list($this->web_partner_id, $this->web_partner_details['id'], $this->web_partner_details['primary_user'], $bookingType, $source);
		}

		$data = [
			'title' => $this->title,
			'view' => "GroupFlight\Views\listing/group-flight-booking-list",
			"list" => $list,
			"search_bar_data" => $getData,
			'pager' => $GroupFlightBookingModel->pager,
		];
	...
	function bookingDetails()
	{
		...
		$amendmentList = array();
		$airlineLogoClass = "";
		$BookingDetail = $GroupFlightBookingModel->flight_booking_detail($this->web_partner_id, $bookingReferenceNumber, $this->web_partner_details['id'], $this->web_partner_details['primary_user']);
		 
		if ($BookingDetail) {
			$amendmentList = $GroupFlightBookingModel->amendment_list($this->web_partner_id, $BookingDetail['id'], $BookingDetail['booking_source']);
			...
			$data = [
				'title' => $this->title,
				'view' => "GroupFlight\Views\listing/group-flight-booking-detail",
	...
	public function amendmentDetails()
	{
		...
		$GroupFlightBookingModel = new GroupFlightBookingModel();
		$BookingDetail = $GroupFlightBookingModel->flight_amendment_detail($this->web_partner_id, $amendmentId);
		...
		if ($amendmentId && $BookingDetail) {
			...
			$data = [
				'title' => $this->title,
				'view' => "GroupFlight\Views\listing/group-flight-amendment-detail",
				"bookingDetail" => $BookingDetail,
			];
			return view('template/sidebar-layout', $data);
		} else {
	...
	public function amendmentStatusChange()
	{
		...
		if (!$rules) {
			...
			if ($data['status'] == "approved") {
				$AmendmentDetail = $GroupFlightBookingModel->flight_amendment_detail($this->web_partner_id, $amendment_id);
				if ($AmendmentDetail['amendment_type'] == "cancellation") {
					...
					foreach ($cancelledPaxIds as $cancelledPaxId) {
						...
						$GroupFlightBookingModel->updateData("group_flight_booking_travelers", array("id" => $cancelledPaxId), $paxUpdatedata);
					}
					$bookingInfo = $GroupFlightBookingModel->getBookingConfirmationData($AmendmentDetail['booking_ref_no']);
					...
					if ($updateflightBookingData) {
						$GroupFlightBookingModel->updateData("group_flight_booking_list", array("id" => $AmendmentDetail['booking_ref_no']), $updateflightBookingData);
					}
				}
			}
			...
			$update = $GroupFlightBookingModel->updateData("group_flight_amendment", array("id" => $amendment_id, "web_partner_id" => $this->web_partner_id), $updateData);
			if ($update) {
				$AmendmentDetail = $GroupFlightBookingModel->amendment_detail($this->web_partner_id, $amendment_id);
				$TravelersInfo = $AmendmentDetail['travellers'];
				$cancelledPaxIds = explode(",", $AmendmentDetail['pax_id']);

				$pax_details = $GroupFlightBookingModel->pax_travellers_details($cancelledPaxIds);
	...
	function amendmentDetail()
	{
		...
		$convertion_rate = 1;
		$GroupFlightBookingModel = new GroupFlightBookingModel();
		$AmendmentDetail = $GroupFlightBookingModel->flight_amendment_detail($this->web_partner_id, $amendmentId);
		$bookingReferenceNumber = $AmendmentDetail['booking_ref_number'];
		if ($amendmentId && $AmendmentDetail) {
			$BookingDetail = $GroupFlightBookingModel->flight_booking_detail($this->web_partner_id, $bookingReferenceNumber, $this->web_partner_details['id'], $this->web_partner_details['primary_user']);

			if ($BookingDetail['booking_currency']) {
				$GroupFlightAmendmentModel = new GroupFlightAmendmentModel();
				$booking_currency = $GroupFlightAmendmentModel->getcurrentcurrencyrates($BookingDetail['booking_currency'], $this->web_partner_id);
				if ($booking_currency) {
			...
			$data = [
				'title' => $this->title,
				'view' => "GroupFlight\Views/flightAmendment/group-flight-amendment-details",
				"bookingDetail" => $BookingDetail,
	...
	function groupflightAmendmentCancellationCharge()
	{
		$data = $this->request->getPost();
		...
		if (!$rules) {
			...
			$GroupFlightAmendmentModel = new GroupFlightAmendmentModel();
			$amendment_id = dev_decode($data['amendment_id']);
			$FlightAmendmentDetail = $GroupFlightAmendmentModel->flight_amendment_detail_by_id($amendment_id, $this->web_partner_id);
			...
			if ($FlightAmendmentDetail) {
				...
				$FlightTravellerDetails = $GroupFlightAmendmentModel->flight_booking_travelers_detail($flight_booking_id, $amendmentpaxIdArray);
				...
				if ($FlightTravellerDetails) {
					...
					if (isset($FlightAmendmentDetail['wl_agent_id']) && $FlightAmendmentDetail['wl_agent_id'] > 0) {
						...
						$agentUserGstCode = $GroupFlightAmendmentModel->agent_user_gst_state_code($tableName, $user_id, $this->web_partner_id);
					} else {
						$tableName = "customer";
					...
					foreach ($FlightTravellerDetails as $paxiteratekey => $FlightTravellerDetail) {
						...
						$GSTInfo = gst_calculate("GroupFlight", $agentUserGstCode, $this->admin_comapny_detail['gst_state_code'], $paxChargeInfo['service_charge']);
					...
					if ($totalRefundAmount > 0) {
						foreach ($paxsRefundChargeDetails as $paxsRefundChargeDetail) {
							...
							$update = $GroupFlightAmendmentModel->updateWithTableData("group_flight_booking_travelers", $updateData, array("id" => $PaxId, "flight_booking_id" => $flight_booking_id));
						}
						...
						$update = $GroupFlightAmendmentModel->updateWithTableData("group_flight_amendment", $updateAmendmentData, array("id" => $amendment_id));
	...
	function groupflightRefundLists()
	{
		$GroupFlightAmendmentModel = new GroupFlightAmendmentModel();
		$getData = $this->request->getGET();
		if (isset($getData['key'])) {
			$list = $GroupFlightAmendmentModel->search_flight_refund_list($this->web_partner_id, $getData);
		} else {
			$source = $this->request->getGET('source');
			if ($source == 'dashboard') {
				$source = 'dashboard';
			...
			$bookingType = 'all';
			$list = $GroupFlightAmendmentModel->flight_refund_list($this->web_partner_id, $bookingType, $source);
		}
		$data = [
			'title' => $this->title,
			'view' => "GroupFlight\Views/flightRefund/group-flight-refund-list",
			"list" => $list,
			"search_bar_data" => $getData,
			'pager' => $GroupFlightAmendmentModel->pager,
		];
		return view('template/sidebar-layout', $data);
	}

	function GroupflightAmendmentLists()
	{
		$GroupFlightAmendmentModel = new GroupFlightAmendmentModel();
		$bookingType = 'all';
		$source = '';
		...
		if (isset($getData['key'])) {
			$list = $GroupFlightAmendmentModel->search_data($getData, $this->web_partner_id, $this->web_partner_details['id'], $this->web_partner_details['primary_user'], $bookingType, $source);
		} else {
			$source = $this->request->getGET('source');
			if ($source == 'dashboard') {
				$source = 'dashboard';
			...
			$list = $GroupFlightAmendmentModel->flight_amendment_list($this->web_partner_id, $this->web_partner_details['id'], $this->web_partner_details['primary_user'], $bookingType, $source);
		}

		$data = [
			'title' => $this->title,
			'view' => "GroupFlight\Views/flightAmendment/group-flight-amendment-list",
			"list" => $list,
			"search_bar_data" => $getData,
			'pager' => $GroupFlightAmendmentModel->pager,
		];
	...
	public function flight_markup()
	{
		if (permission_access_error("Flight", "flight_markup_list")) {
			$GroupFlightMarkupModel = new GroupFlightMarkupModel();

			if ($this->request->getGet() && $this->request->getGet('key')) {
				$lists = $GroupFlightMarkupModel->search_data($this->request->getGet(), $this->web_partner_id);
			} else {
				$lists = $GroupFlightMarkupModel->markup_list($this->web_partner_id);
			}
			...
			$data = [
				...
				'view' => "GroupFlight\Views\Flight-markup-list",
				'pager' => $GroupFlightMarkupModel->pager,
	...
	function GetUpdateFlightTicketInfo()
	{
		...
		$GroupFlightBookingModel = new GroupFlightBookingModel();
		$amendmentList = array();
		$bookingInfo = $GroupFlightBookingModel->flight_booking_detail($this->web_partner_id, $booking_refrence_number, $this->web_partner_details['id'], $this->web_partner_details['primary_user']);
		/* pr($bookingInfo);exit; */
		$flightSupplier = $GroupFlightBookingModel->getData("offline_provider", array('web_partner_id' => $this->web_partner_id, 'flight_service' => 'active'), $singalRecord = 0, $whereApply = 1, 'id,supplier_name');
		if ($bookingInfo) {
			$amendmentList = $GroupFlightBookingModel->amendment_list($this->web_partner_id, $bookingInfo['id'], $bookingInfo['booking_source']);
