...
use App\Modules\Query\Models\WeddingqueryModel;
...
class Query extends BaseController
{
    ...
    public function wedding_query_list()
    {
        $WeddingqueryModel = new WeddingqueryModel();
        $getData = $this->request->getGET();
        if (isset($getData['key'])) {
            $weddingData = $WeddingqueryModel->search_data($this->web_partner_id, $getData);
        } else {
            $weddingData = $WeddingqueryModel->wedding_query_list($this->web_partner_id);
        }

        $data = [
            'title' => 'Wedding Query List',
            'weddinglist' => $weddingData,
            "search_bar_data" => $getData,
            'pager' => $WeddingqueryModel->pager,
            'view' => 'Query\Views\wedding-query-list'
        ];
        return view('template/sidebar-layout', $data);
    }

    public function weddingquery_list_details()
    {

        $uri = $this->request->getUri();
        $weddingquery_id =  dev_decode($uri->getSegment(3));
        $WeddingqueryModel = new WeddingqueryModel();
        $details = $WeddingqueryModel->weddingquery_list_details($weddingquery_id, $this->web_partner_id);
        $data = [
            'title' => $this->title,
            'id' => $weddingquery_id,
            'details' => $details,
        ];
        $blog_details = view('Modules\Query\Views\wedding-query-details', $data);
        $data = array("StatusCode" => 0, "Message" => $blog_details, 'class' => 'success_popup');
        return $this->response->setJSON($data);
    }

    public function remove_wedding_query_list()
    {
        $WeddingqueryModel = new WeddingqueryModel();
        $ids = $this->request->getPost('checklist');
        foreach ($ids as $id) {

            $delete = $WeddingqueryModel->remove_query($id, $this->web_partner_id);
        }

        if ($delete) {
            $message = array("StatusCode" => 0, "Message" => "Wedding Query  Successfully  Deleted", "Class" => "success_popup", "Reload" => "true");
        } else {
            $message = array("StatusCode" => 2, "Message" => "Wedding Query  not Deleted", "Class" => "error_popup", "Reload" => "true");
        }
        $this->session->setFlashdata('Message', $message);
        return $this->response->setJSON($message);
    }
