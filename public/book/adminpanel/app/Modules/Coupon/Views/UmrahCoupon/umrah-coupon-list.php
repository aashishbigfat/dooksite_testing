        ...
        <div class="table_title">
            <div class="sale_bar">
                <div class="row align-items-center">
                    <div class="col-md-4">
                        <h5 class="m-0">Umrah Coupon</h5>
                    </div>
                    <div class="col-md-8 text-end">
                        <?php if (permission_access("Coupon", "coupon_umrah_add")) { ?>
                            <button class="badge badge-wt" view-data-modal="true" data-controller='flight'
                                data-href="<?= site_url('coupon/umrah-coupon-view') ?>"><i class="fa-solid fa-add"></i> Add
                                Umrah Coupon
                            </button>
                        <?php } ?>

                        <?php if (permission_access("Coupon", "coupon_umrah_change_status")) { ?>
                            <button class="badge badge-wt" onclick="confirm_change_status('status_change')"><i
                                    class="fa-solid fa-exchange"></i> Change Status
                            </button>
                        <?php } ?>
                        <?php if (permission_access("Coupon", "coupon_umrah_delete")) { ?>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <form action="<?= site_url('coupon/umrah-coupon'); ?>" method="GET" class="tts-dis-content row mb-3"
                        ...
                        <div class="col-md-3">
                            <div class="form-group form-mb-20">
                                ...
                                <select name="key" class="form-select" onchange="tts_searchkey(this,'markup-search')"
                                    ...
                                    <option value="code" <?= !empty($search_bar_data['key']) && $search_bar_data['key'] === 'code' ? 'selected' : '' ?>>
                                        Code
                                    </option>
                                    <option value="date-range" <?= !empty($search_bar_data['key']) && $search_bar_data['key'] === 'date-range' ? 'selected' : '' ?>>
                                        Date Range
                                    </option>
                                </select>
                            </div>
                        ...
                        <div class="col-md-2">
                            <?php if (!empty($search_bar_data['key'])): ?>
                                <div class="search-reset-btn">
                                    <a href="<?= site_url('coupon/umrah-coupon'); ?>">Reset Search</a>
                                </div>
                            <?php endif ?>
                        </div>
                    </form>
                ...
                <?php
                $trash_uri = "coupon/remove-umrah-coupon";
                ?>
                <form action="<?= site_url($trash_uri); ?>" method="POST" tts-form="true" id="formmarkuplist">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover">
                            <thead class="table-active">
                                <tr>
                                    <?php if (permission_access("Coupon", "coupon_umrah_change_status") || permission_access("Coupon", "coupon_umrah_delete")) { ?>
                                    ...
                                    <th>Status</th>
                                    <th>Created</th>
                                    <?php if (permission_access("Coupon", "coupon_umrah_details_list")) { ?>
                                        <th>Action</th>
                                    <?php } ?>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                if (!empty($list) && is_array($list)) {
                                    foreach ($list as $data) {
                                        $class = !empty($data['status']) && ($data['status'] == 'active') ? 'active-status' : 'inactive-status';
                                        ?>
                                        <tr>
                                            <td>
                                                <label>
                                            ...
                                            <td>
                                                <a href="javascript:void(0);" view-data-modal="true" data-controller='coupon'
                                                    ...
                                                    data-href="<?= site_url('coupon/coupon-umrah-details/') . dev_encode($data['id']); ?>">
                                                    <?= ucfirst($data['code']); ?></a>
                                            </td>

                                            <td>
                                                <?= !empty($data['travel_date_from']) ? date('d M,Y', $data['travel_date_from']) : '-'; ?>
                                            </td>
                                            <td>
                                                <?= !empty($data['travel_date_from']) ? date('d M,Y', $data['travel_date_to']) : '-'; ?>
                                            </td>

                                            <td>
                                                <?= !empty($data['coupon_type']) ? ucfirst($data['coupon_type']) : '-'; ?>
                                            </td>
                                            ...
                                            <td>
                                                <?= !empty($data['max_limit']) ? $data['max_limit'] : '-'; ?>
                                            </td>

                                            <!-- <td>
                                                <?= !empty($data['theme_name']) ? $data['theme_name'] : '-'; ?>
                                            </td>
                                            <td>
                                                <?= !empty($data['destination_name']) ? $data['destination_name'] : '-'; ?>
                                            </td> -->
                                            ...
                                            <td>
                                                <?= !empty($data['created']) ? date_created_format($data['created']) : '-'; ?>
                                            </td>
                                            <?php if (permission_access("Coupon", "coupon_umrah_details_list")) { ?>
                                                <td>
                                                    <a href="javascript:void(0);" view-data-modal="true" data-controller='coupon'
                                                        data-id="<?= dev_encode($data['id']); ?>"
                                                        data-href="<?= site_url('coupon/coupon-umrah-details/') . dev_encode($data['id']); ?>">View</a>
                                                </td>
                                            <?php } ?>
                                    ...
                                    echo "<tr> <td colspan='12' class='text-center'><b>No  Umrah Coupon Found</b></td></tr>";
                                } ?>
                            </tbody>
                        </table>
                    </div>
...
<div id="status_change" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form action="<?= site_url('coupon/umrah-coupon-status-change'); ?>" method="post" tts-form="true"
