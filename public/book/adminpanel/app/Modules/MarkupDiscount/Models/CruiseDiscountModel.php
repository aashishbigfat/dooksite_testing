...
class CruiseDiscountModel extends Model
{
    ...
    public function super_admin_cruise_discount_list($web_partner_id)
    {
        return  $this->select('web_partner_cruise_discount.*')->where('web_partner_id',$web_partner_id)

            ->orderBy("id","DESC")->paginate(40);
    }

    public function super_admin_cruise_discount_details($id)
    {
        return  $this->select('web_partner_cruise_discount.*')->where("id",$id)->get()->getRowArray();
    }
    ...
    function search_data($web_partner_id,$data)
    {
        if ($data['from_date'] && $data['to_date']) {
            ...
            if ($data['key'] == 'date-range') {
                ...
                return  $this->select('web_partner_cruise_discount.*')
