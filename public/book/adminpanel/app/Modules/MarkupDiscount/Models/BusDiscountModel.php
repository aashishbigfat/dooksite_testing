...
class BusDiscountModel extends Model
{
    protected $table = 'web_partner_bus_discount';
    protected $primarykey = 'id';
    protected $protectFields = false;


    public function super_admin_bus_discount_list($web_partner_id)
    {
        return  $this->select('web_partner_bus_discount.*')->where('web_partner_id',$web_partner_id)

            ->orderBy("web_partner_bus_discount.id","DESC")->paginate(40);
    }

    public function super_admin_bus_discount_details($id)
    {
        return  $this->select('web_partner_bus_discount.*')->where("id",$id)->get()->getRowArray();
    }
