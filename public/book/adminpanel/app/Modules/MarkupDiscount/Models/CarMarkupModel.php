...
class CarMarkupModel extends Model
{
    protected $table = 'web_partner_car_markup';
    protected $primarykey = 'id';
    protected $protectFields = false;


    public function super_admin_car_markup_list($web_partner_id)
    {
        return  $this->select('web_partner_car_markup.*')->where('web_partner_id',$web_partner_id)

            ->orderBy("web_partner_car_markup.id","DESC")->paginate(40);
    }

    public function super_admin_car_markup_details($id)
    {
        return  $this->select('web_partner_car_markup.*')->where("id",$id)->get()->getRowArray();
    }
