function checkBiketourSearchValidation() {
  setTimeout(function () {
    $(".error-message").remove();
    var form = $("[name  =  'biketour-form']");
    if ($("[name  =  'biketour-form']").find('.error').length == 0) {
      ...
      $.ajax({
        url: site_url + 'biketour/biketour-check-search-validation',
        dataType: "json",
        type: "POST",
        cache: false,
        data: form.serialize(),
...
$(document).on("click", "[tts-biketour-destination]", function (event) {
  $(event.target).autocomplete({
    minLength: 0,
    maxResults: 15,
    source: function (request, response) {
      $.ajax({
        url: site_url + 'biketour/get-city-biketour',
        dataType: 'json',
        data: {
    ...
    select: function (event, ui) {
      ...
      $("[tts-biketour-location_id]").val(ui.item.location_id);
      return false;
    },
