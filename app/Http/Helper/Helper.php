...
if (!function_exists('poiDepartureCountsUnfiltered')) {
    /**
     * Total and featured departure counts for a set of POI reference ids.
     *
     * Replaces a query per POI plus a further query per linked departure with
     * a single grouped query. Counts rows rather than distinct departures, so
     * a departure linked to the same POI twice counts twice - that is what the
     * loop this came from did, and the figures on the page depend on it.
     * "Unfiltered" means no status condition on either table, again matching
     * the original: only featured = 1 is applied, when counting featured.
     *
     * @param  array  $poiIds
     * @return array  keyed by reference id => ['total' => int, 'featured' => int]
     */
    function poiDepartureCountsUnfiltered(array $poiIds)
    {
        // Memoised for the request: the country and region pages ask for
        // overlapping POI sets more than once while rendering.
        static $memo = [];

        $empty = ['total' => 0, 'featured' => 0];
        $counts = [];
        $wanted = [];
        ...
        $rows = DB::table('departure_destination_point_of_interests as poi')
            ->leftJoin('departures as d', 'd.id', '=', 'poi.departure_id')
            ->whereIn('poi.reference_id', array_values($wanted))
            ->groupBy('poi.reference_id')
            ->select(
                'poi.reference_id',
                DB::raw('COUNT(*) as total'),
                DB::raw('SUM(CASE WHEN d.featured = 1 THEN 1 ELSE 0 END) as featured')
            )
            ->get();

        $fetched = [];
        foreach ($rows as $row) {
            $fetched[(string) $row->reference_id] = [
                'total'    => (int) $row->total,
                'featured' => (int) $row->featured,
            ];
        }

        // A POI with no rows at all is absent from the result set; record the
        // zero so it is not looked up again.
        foreach ($wanted as $key => $poiId) {
            $value = isset($fetched[$key]) ? $fetched[$key] : $empty;
            $memo[$key] = $value;
            $counts[$poiId] = $value;
        }
